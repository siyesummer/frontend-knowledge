...
- 当前生产版本为 `v0.10.0`，镜像 `siye-prod-agent:0.10.0`；容器只加入 edge 网络，不发布宿主机 `3002`。生产 Hybrid 索引为 2005 分片，代码围栏内的 `#` 注释不再被误当 Markdown 标题。
...
- 用户显式“有帮助/没帮助”已在 Agent `v0.11.0` 与 Widget `v0.3.0` 发布候选实现，尚未生产启用：回答完成后 Widget 才显示反馈；浏览器只持有与 `requestId` 绑定的 HMAC 凭证，不持有追踪查询 Token；负反馈只选择结构化原因，不收集自由文本。服务端按回答幂等更新并把追加式事件写入独立持久卷，受控查询仍使用 `X-RAG-Trace-Token`。该信号只能进入人工审核，不能自动成为评测真值或训练数据；发布前必须先建立反馈卷备份，再验证伪造签名拒绝、重复提交、改选、容器重建恢复和公网查询隐藏。
